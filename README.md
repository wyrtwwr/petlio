...
Новые фото сохраняются вне публичного корня в `PET_PHOTO_STORAGE_PATH`, а в заказе хранятся ссылки вида `private:<имя>`. Основное фото обязательно для всех дизайнов. Для дизайна «Идентификация питомца» можно добавить второе фото: его путь записывается в `pet_secondary_photo_path`, а после подтверждения оплаты оба найденных файла прикладываются к письму владельцу. Выбранный дизайн сохраняется в очищенном `raw_payload` с каноническими серверными ключом и названием и выводится в текстовой и HTML-версии письма. Основное фото выдаётся через `my-orders/photo.php` только после проверки серверной сессии и совпадения email заказа. Для старых файлов в `backend/uploads/order-photos/` оставлена совместимость; Apache закрывает каталог через `.htaccess`, а в Nginx нужно запретить URL `/backend/uploads/`. `checkout_request_id` и уникальный индекс делают повтор одного запроса идемпотентным: для него не создаётся второй заказ или вторая платёжная ссылка.
...
2. До открытия обновлённого сайта для заказов последовательно применить `migrations/2026_robokassa.sql`, `migrations/2026_my_orders.sql`, `migrations/2026_delivery_price.sql` и `migrations/2026_secondary_pet_photo.sql`, если база уже существовала. Проверить `SHOW COLUMNS FROM orders LIKE 'pet_secondary_photo_path';`: строка должна существовать до первого заказа на новой версии кода.
...
6. Проверить тестовый платёж с `ROBOKASSA_TEST=true`: название каждого из трёх дизайнов в письме владельцу, одно вложение для первых двух дизайнов, два вложения при добавлении второго фото в третьем дизайне, письмо покупателю, magic-link вход, список и закрытость фото другого клиента.
